...
cd $ROOT_DIR
mace_run_train \
    --name="stress${stress}_nc${NUM_CHANNEL}_nr${NUM_RADIAL}_MLP${MLP_IRREPS}_b${REAL_BATCH_SIZE}_lr$2_${conf_str}_intfirst-${int_first}_int-${int}x${num_int}i_maxL${max_L}_maxell${max_ell}_rsmooth" \
    --loss='universal' \
    --energy_weight=1 \
    --forces_weight=10 \
    --compute_stress=True \
    ...
    --checkpoints_dir checkpoints/mp_rsmooth \
    --rsmooth 0.01 \
    #--clean_alex \
