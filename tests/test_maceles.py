...
@pytest.mark.skipif(not LES_AVAILABLE, reason="LES library is not available")
def test_run_train_with_mp(tmp_path, fitting_configs) -> None:
    ase.io.write(tmp_path / "fit.xyz", fitting_configs)
...
def test_run_train_maceles_cueq(tmp_path, fitting_configs) -> None:
    ase.io.write(tmp_path / "fit.xyz", fitting_configs)
...
@pytest.mark.skipif(not LES_AVAILABLE, reason="LES library is not available")
def test_run_eval_with_bec(tmp_path: Path, maceles_model_path: Path, fitting_configs) -> None:
...
def test_run_eval_fail_with_wrong_model(
    ...
    with pytest.raises(
        ValueError, match=r"BEC can only be computed with MACELES model.",
    ):
        mace_eval_configs_run(args)


@pytest.mark.skipif(not LES_AVAILABLE, reason="LES library is not available")
def test_run_eval_no_bec(tmp_path: Path, maceles_model_path: Path, fitting_configs) -> None:
