...
def atoms_from_config(config):
    atom = Atoms(config.atomic_numbers)
    atom.set_positions(config.positions)
    return atom
...
def main() -> None:
    ...
    new hdf5 file that is ready for training with on-the-fly dataloading
    """
    args = tools.build_preprocess_distill_arg_parser().parse_args()
    run(args)
...
def run(args: argparse.Namespace):
    ...
    # # collections.train: List[mace.data.utils.Configuration]
    if args.calculator == "mace_off":
        calculator = mace_off(model=args.calculator_size, device=args.device)
    elif args.calculator == "mace_mp":
        calculator = mace_mp(model=args.calculator_size, device=args.device)
    
    # for config in tqdm.tqdm(collections.train): # shards (nvidia package) TODO
    #     atoms = atoms_from_config(config)
    #     descriptors, node_energy = calculator.get_descriptors(atoms, invariants_only=True, node_energy=True)
    #     config.descriptors, config.node_energy = descriptors, node_energy
    
    # for config in tqdm.tqdm(collections.valid):
    #     atoms = atoms_from_config(config)
    #     descriptors, node_energy = calculator.get_descriptors(atoms, invariants_only=True, node_energy=True)
    #     config.descriptors, config.node_energy = descriptors, node_energy
    ...
    if args.test_file is not None:
        ...
        for name, subset in collections.tests:
            ...
            for config in tqdm.tqdm(subset):
                atoms = atoms_from_config(config)
                descriptors, node_energy = calculator.get_descriptors(atoms, invariants_only=True, node_energy=True)
                config.descriptors, config.node_energy = descriptors, node_energy
