...
| R4 | **Debt book**: every temporary compromise is a row tied to the ticket whose merge removes it. The ledger is **internal** and carries no dates: there is no public EOL promise, `main` stays on v0.3 until v1 is released, and no end date for v0.3 is announced. | One fitness function per row, `xfail(strict=True)` while the row is open, so the row goes **red** the moment its burn step lands and nobody deleted it. |
...
- [ ] From Phase 3 onward, CI requires `git ls-files mace/` to be **strictly non-increasing on `mace-reforge`**, one step per burn-step ticket in the debt book. It says nothing about `develop`, where `mace/` keeps growing until the merge back.
