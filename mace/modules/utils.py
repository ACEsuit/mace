...
def compute_forces(energy: torch.Tensor, positions: torch.Tensor, training: bool = True) -> torch.Tensor:
...
def compute_hessians_vmap(
    ...
    try:
        ...
        gradient = torch.vmap(get_vjp, in_dims=0, out_dims=0, chunk_size=chunk_size)(I_N)[0]
    except RuntimeError:
...
def extract_invariant(x: torch.Tensor, num_layers: int, num_features: int, l_max: int):
    ...
    for i in range(num_layers - 1):
        out.append(
            x[
                :,
                i * (l_max + 1) ** 2 * num_features : (i * (l_max + 1) ** 2 + 1) * num_features,
            ]
        )
...
def compute_fixed_charge_dipole(
    ...
    return scatter_sum(src=mu, index=batch.unsqueeze(-1), dim=0, dim_size=num_graphs)  # [N_graphs,3]
