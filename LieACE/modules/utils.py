...
def create_U_element(node_deg, n_elements, i, A, device):
    ...
    for l in range(1, vu + 1):
        U, index_mm = create_U(
            A, l, node_deg[i]
        )  # create the U matrix of the speficied element different degrees for different correlation
        U = expand_element(
            U.coalesce(), n_elements
        )  # make copies to match the number of elements
...
class c_tildes_weight(torch.nn.Module):
    def __init__(self, c_tildes_dict, num_elements: int, device="cpu"):
        super().__init__()
        ...
        for vu in range(2, self.max_corr + 1):
            ...
            self.size[vu] = tuple(
                (((nmax + 1) * (lmax + 1) ** 2) * num_elements for i in range(vu))
            )

    def forward(self, c_tildes_dict):
        c_tildes_dict_w = {}  # initialize the weighted dict of c_tildes
        c_tildes_dict_w["degree"] = c_tildes_dict[
            "degree"
        ]  # pass down the dgree argument
        c_tildes_dict_w[1] = spmm(
            c_tildes_dict[1].transpose(0, 1)._indices(),
            c_tildes_dict[1]._values(),
            c_tildes_dict[1].size()[1],
            c_tildes_dict[1].size()[0],
            self.weights[str(1)],
        ).to_sparse()
        for vu in range(2, self.max_corr + 1):
            c_tildes_dict_w[vu] = (
                spmm(
                    c_tildes_dict[vu]._indices(),
                    c_tildes_dict[vu]._values(),
                    c_tildes_dict[vu].size()[0],
                    c_tildes_dict[vu].size()[1],
                    self.weights[str(vu)],
                )
                .to_sparse()
                .coalesce()
            )
            c_tildes_dict_w[vu] = unfold(
                fold(c_tildes_dict_w[vu], 0, self.size[vu], device=self.device),
                0,
                device=self.device,
            )
