...
class SymmetricContraction(CodeGenMixin, torch.nn.Module):
    def __init__(
        ...
        contraction_type: str = "base",
    ) -> None:
        super().__init__()
        self.contraction_type = contraction_type
        CONTRACTION_CLS = {
            "base": Contraction,
            "new": LowOrderContraction,
        }
        ...
        self.contraction_cls = CONTRACTION_CLS[self.contraction_type]
        for irrep_out in self.irreps_out:
...
@compile_mode("script")
class SymmetricMultiContraction(CodeGenMixin, torch.nn.Module):
    def __init__(
        self,
        irreps_in: o3.Irreps,
        irreps_out: o3.Irreps,
        ...
        self.contractions = torch.nn.ModuleList()
        self.contraction_cls = MultiContraction
        for irrep_out in self.irreps_out:
            self.contractions.append(
                self.contraction_cls(
                    irreps_in=self.irreps_in,
...
@compile_mode("script")
class LowOrderContraction(torch.nn.Module):
    def __init__(
        self,
        irreps_in: o3.Irreps,
        irrep_out: o3.Irreps,
...
@compile_mode("script")
class MultiContraction(torch.nn.Module):
    def __init__(
        self,
        irreps_in: o3.Irreps,
        irrep_out: o3.Irreps,
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        assert x.dim() == 4, "MultiContraction Takes cor, b, c, i node_features"
        out = self.graph_opt_main(
            self.U_tensors(self.correlation),
            self.weights_max,
            x[:, 0],
        ...
        for i, (weight, contract_weights, contract_features) in enumerate(
            ...
            c_tensor = c_tensor + out
            out = contract_features(c_tensor, x[:, i+1])
