...
from LieCG.CG_coefficients.CG_rot import (
    ClebschGordan,
    Rot3DCoeffs,
    clebschgordan,
    re_basis,
    ...
    while ntest < 200:
        ...
        for m2 in range(-1 * j2, j2 + 2):
            ...
            if abs(M) <= J:
                ...
                assert np.isclose(
                    cg_eval(inds),
                    spin.CG(j1, m1, j2, m2, J, M).doit().evalf().__float__(),
                    atol=1e-7,
                )
...
def test_sph_exp():
    ...
    for ntest in range(200):
        ...
        p = (
            Ylm_func.compute_ylm(l1, m1, R)[0] * Ylm_func.compute_ylm(l2, m2, R)[0]
        ...
        for L in range(abs(M), l1 + l2 + 1):
            p2 += (
                np.sqrt((2 * l1 + 1) * (2 * l2 + 1) / (4 * np.pi * (2 * L + 1)))
                * clebschgordan(l1, 0, l2, 0, L, 0)
                * clebschgordan(l1, m1, l2, m2, L, M)
                * Ylm_func.compute_ylm(L, M, R)[0]
            )
        assert np.isclose(p, p2, atol=1e-7)
...
def test_Rot3Dcoeffs():
    ...
    assert np.isclose(
        A(ll, mm, kk), -0.030116930096841705, atol=1e-7
    )  # number from ACE.jl
