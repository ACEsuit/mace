...
| `extra.cueq-cuda-11` | `[cueq-cuda-11]` | `setup.cfg` | KEEP — idem; the ops major must match `torch.version.cuda`, not the newest available | `tests/unit/test_ci_gates.py::test_the_job_reads_the_extras_out_of_setup_cfg` (the `cueq-wheel-extras` PR job resolves each one; this keeps setup.cfg and that job in step) |
| `extra.cueq-cuda-12` | `[cueq-cuda-12]` | `setup.cfg` | KEEP — idem | `tests/unit/test_ci_gates.py::test_the_job_reads_the_extras_out_of_setup_cfg` (the `cueq-wheel-extras` PR job resolves each one; this keeps setup.cfg and that job in step) |
