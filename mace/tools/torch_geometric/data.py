    ...
    def debug(self):
        if self.edge_index is not None:
            if self.edge_index.dtype != torch.long:
                raise RuntimeError(
                    f"Expected edge indices of dtype {torch.long}, but found dtype " f" {self.edge_index.dtype}"
                )

        if self.face is not None:
            if self.face.dtype != torch.long:
                raise RuntimeError(
                    f"Expected face indices of dtype {torch.long}, but found dtype " f" {self.face.dtype}"
                )

        if self.edge_index is not None:
            if self.edge_index.dim() != 2 or self.edge_index.size(0) != 2:
                raise RuntimeError(
                    "Edge indices should have shape [2, num_edges] but found" f" shape {self.edge_index.size()}"
                )
        ...
        if self.edge_index is not None and self.num_nodes is not None:
            ...
            if min_index < 0 or max_index > self.num_nodes - 1:
                raise RuntimeError(
                    f"Edge indices must lay in the interval [0, {self.num_nodes - 1}]" f" but found them in the interval [{min_index}, {max_index}]"
                )

        if self.face is not None:
            if self.face.dim() != 2 or self.face.size(0) != 3:
                raise RuntimeError(
                    "Face indices should have shape [3, num_faces] but found" f" shape {self.face.size()}"
                )
        ...
        if self.face is not None and self.num_nodes is not None:
            ...
            if min_index < 0 or max_index > self.num_nodes - 1:
                raise RuntimeError(
                    f"Face indices must lay in the interval [0, {self.num_nodes - 1}]" f" but found them in the interval [{min_index}, {max_index}]"
                )

        if self.edge_index is not None and self.edge_attr is not None:
            if self.edge_index.size(1) != self.edge_attr.size(0):
                raise RuntimeError(
                    "Edge indices and edge attributes hold a differing " f"number of edges, found {self.edge_index.size()} and {self.edge_attr.size()}"
                )

        if self.x is not None and self.num_nodes is not None:
            if self.x.size(0) != self.num_nodes:
                raise RuntimeError(
                    f"Node features should hold {self.num_nodes} elements in the first " f"dimension but found {self.x.size(0)}"
                )

        if self.pos is not None and self.num_nodes is not None:
            if self.pos.size(0) != self.num_nodes:
                raise RuntimeError(
                    f"Node positions should hold {self.num_nodes} elements in the first " f"dimension but found {self.pos.size(0)}"
                )

        if self.normal is not None and self.num_nodes is not None:
            if self.normal.size(0) != self.num_nodes:
                raise RuntimeError(
                    f"Node normals should hold {self.num_nodes} elements in the first " f"dimension but found {self.normal.size(0)}"
                )
