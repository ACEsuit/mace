...
def run_compiled(model, *args, **kwargs):
    """Execute a torch.compile'd model, converting a broken-toolchain Inductor
    failure into a skip. Some GitHub runner images intermittently ship a g++
    that cannot exec cc1plus; that is an infrastructure flake, not a MACE
    regression, and it also breaks the OLD CI the same way."""
    try:
        return model(*args, **kwargs)
    except Exception as exc:  # pylint: disable=broad-except
        if "cc1plus" in str(exc):
            pytest.skip("runner C++ toolchain broken (g++ cannot exec cc1plus)")
        raise
...
def test_mace(device, default_dtype):  # pylint: disable=W0621
    ...
    output2 = run_compiled(model_compiled, batch2, training=True)
...
def test_mace_compile_stress(device, default_dtype):  # pylint: disable=W0621
    ...
    output_compiled = run_compiled(
        model_compiled, batch_compiled, training=True, compute_stress=True
    )
...
@pytest.mark.parametrize(
    "enable_cueq", [pytest.param(True, marks=pytest.mark.cueq), False]
)
...
@pytest.mark.parametrize(
    "enable_cueq", [False, pytest.param(True, marks=pytest.mark.cueq)]
)
