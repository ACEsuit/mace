...
def krylov_constraint_solve_upto_r(C,r,tol=1e-5,lr=1e-2):#,W0=None):
    ...
    W0 = torch.randn((C.shape[-1],r),dtype=torch.cfloat)/torch.sqrt(torch.tensor(C.shape[-1]))# if W0 is None else W0
    ...
    scutoff = (S[rank] if r>rank and S.shape != torch.Size([1]) else 0)
...
print(Clebsch_Gordan(Group=SU(2),lmax=3,irreps=SU2Irreps).dict())