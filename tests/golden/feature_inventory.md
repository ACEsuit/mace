...
| `extra.cueq-cuda-11` | `[cueq-cuda-11]` | `setup.cfg` | KEEP — idem; the ops major must match `torch.version.cuda`, not the newest available | `tests/unit/test_ci_gates.py::test_every_cueq_extra_is_in_the_nightly_loop` (the `cueq-wheel-extras` PR job resolves each one; this keeps setup.cfg and that job in step) |
| `extra.cueq-cuda-12` | `[cueq-cuda-12]` | `setup.cfg` | KEEP — idem | `tests/unit/test_ci_gates.py::test_every_cueq_extra_is_in_the_nightly_loop` (the `cueq-wheel-extras` PR job resolves each one; this keeps setup.cfg and that job in step) |
| `extra.cueq-cuda-13` | `[cueq-cuda-13]` | `setup.cfg` | KEEP — idem; cu13 ops start at cuequivariance 0.7.0 | `tests/unit/test_ci_gates.py::test_every_cueq_extra_is_in_the_nightly_loop` (the `cueq-wheel-extras` PR job resolves each one; this keeps setup.cfg and that job in step) |
