...
python mace/cli/run_train.py \
    --name="stress${stress}_nc${NUM_CHANNEL}_nr${NUM_RADIAL}_MLP${MLP_IRREPS}_b${REAL_BATCH_SIZE}_lr$2_${conf_str}_intfirst-${int_first}_int-${int}x${num_int}i_maxL${max_L}_maxell${max_ell}_twohead" \
    --loss='universal' \
    --energy_weight=10 \
    --forces_weight=1 \
    --compute_stress=True \
    ...
    --checkpoints_dir refit_balanced_reduce_foundation_forceweight1 \
    --foundation_model "/lustre/fswork/projects/rech/gax/unh55hx/mace/checkpoints/salex_mp_from_omat_v9/omat_refit_e0s.model" \
    #--clean_alex \
