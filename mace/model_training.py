...
def train_mace_model(args: Namespace) -> None:
    """Run MACE model training

    This is the main fitting program, assumes that the `args` passed in are correctly
    parsed arguments according to the parser defined in tools. This can also be
    simulated through constructing a `argparse.Namespace` object with the desired
    keywords.
