...
def _subsample_data(
    ...
    if subselect == SubselectType.FPS:
        ...
        logging.info(f"Selecting {num_samples} configurations out of {len(filtered_atoms)} using Farthest Point Sampling")
        return _maybe_fps(filtered_atoms, num_samples)
    if subselect == SubselectType.RANDOM:
        logging.info(f"Subselecting {num_samples} from filtered {len(filtered_atoms)} using random sampling")
