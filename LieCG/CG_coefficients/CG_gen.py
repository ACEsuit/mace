...
class Clebsch_Gordan:
    ...
    def constraint_matrix(self, l1, l2, l3):
        ...
        A.extend(
            [
                LazyKronsum([self.irreps_l1.drho(Ti), self.irreps_l2.drho(Ti)])
                for Ti in self.lie_generators
            ]
        )  # replace with lazy for saving cost
        A.extend(
            [
                LazyKron(self.irreps_l1.rho(hi), self.irreps_l2.rho(hi))
                for hi in self.discrete_generators
            ]
    ...
    def dict(self):
        ...
        l_tuples = list(
            i
            for i in itertools.product([k for k in range(self.lmax)], repeat=3)
            if i[0] >= i[1]
        )
...
class Clebsch_Gordan_n:
    ...
    def constraint_matrix(self, l1, l2, l3):
        ...
        A.extend(
            [
                kronsum(self.irreps_l1.drho(Ti), self.irreps_l2.drho(Ti))
                for Ti in self.lie_generators
            ]
        )  # replace with lazy for saving cost
        A.extend(
            [
                torch.kron(self.irreps_l1.rho(hi), self.irreps_l2.rho(hi))
                for hi in self.discrete_generators
            ]
        )  # replace with lazy for cost
        # replace with lazy for cost
        ...
        Q.extend(
            [
                torch.kron(-Ai.T.contiguous(), In3) + torch.kron(In1n2, Bi.contiguous())
...
def krylov_constraint_solve_upto_r(C, r, tol=1e-6, lr=1e-2):  # ,W0=None):
    ...
    W0 = torch.randn((C.shape[-1], r)) / torch.sqrt(
    ...
    pbar = tqdm(
        total=100,
        desc=f"Krylov Solving for CG Coefficients r<={r}",
    ...
    for i in range(200000):
        ...
        # update progress bar
        progress = max(
            100 * torch.log(lossval / lstart) / torch.log(tol ** 2 / lstart) - prog_val,
            0,
        )
