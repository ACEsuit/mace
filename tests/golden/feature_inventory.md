...
|---|---|---|---|---|
| `ep.mace_run_train` | `mace.cli.run_train:main` | `packages/mace-launcher/pyproject.toml` | KEEP — becomes `mace train` | `tests/workflows/test_cli_contracts.py::test_training_reduces_the_validation_loss_and_writes_a_model` |
| `ep.mace_eval_configs` | `mace.cli.eval_configs:main` | `packages/mace-launcher/pyproject.toml` | KEEP — the capability survives, but the CLI shrinks to a thin `mace eval` adapter over the ASE calculator (read configs → calculator → write XYZ); most of its ~400 lines is argparse duplicating what the calculator already does, and one numerical path is cheaper to pin than two | `tests/workflows/test_cli_contracts.py::test_eval_configs_reproduces_the_committed_anchor_reference` |
| `ep.mace_prepare_data` | `mace.cli.preprocess_data:main` | `packages/mace-launcher/pyproject.toml` | KEEP — becomes `mace data prepare` | `tests/workflows/test_preprocess.py` |
| `ep.mace_create_lammps_model` | `mace.cli.create_lammps_model:main` | `packages/mace-launcher/pyproject.toml` | KEEP — becomes `mace export lammps` | `tests/integrations/lammps/test_export_golden.py::test_the_exported_artifact_reproduces_the_committed_numbers` |
| `ep.mace_select_head` | `mace.cli.select_head:main` | `packages/mace-launcher/pyproject.toml` | KEEP — becomes `mace model select-head` | `tests/workflows/test_cli_contracts.py::test_select_head_lists_the_heads_of_a_multihead_model` + `tests/workflows/test_cli_contracts.py::test_select_head_and_the_multihead_model_agree_on_the_selected_head` |
| `ep.mace_plot_train` | `mace.cli.plot_train:main` | `packages/mace-launcher/pyproject.toml` | KEEP — reduced: the basic plot subcommand stays (per-head loss curves, cheap over a structured log); `--plot_interaction_e` goes (§3.1) | `tests/workflows/test_plot_train.py::test_a_real_results_log_becomes_a_plot` |
| `ep.mace_polar_density_cube` | `mace.cli.polar_density_cube:main` | `packages/mace-launcher/pyproject.toml` | KEEP — the electrostatics extra's density-cube tool | `tests/extensions/polar/test_polar_density_cube.py` (port cases) |
| `ep.mace_finetuning_select` | `mace.cli.fine_tuning_select:main` | `packages/mace-launcher/pyproject.toml` | MERGE — absorbed into the integrated fine-tuning pipeline (`mace train` with a fine-tuning config) | `tests/workflows/test_finetuning_contracts.py::test_subselect_fps_uses_the_model_and_still_returns_the_requested_number` |
| `ep.mace_convert_device` | `mace.cli.convert_device:main` | `packages/mace-launcher/pyproject.toml` | KEEP — becomes `mace model convert-device`; explicitly **not** one of the five weight converters, because it converts device/dtype and not backend layout | `tests/unit/test_scale_shift_dtype.py::test_the_convert_device_cli_preserves_the_buffers` |
| `ep.mace_e3nn_cueq` | `mace.cli.convert_e3nn_cueq:main` | `packages/mace-launcher/pyproject.toml` | DROP — v1 weights are canonical and backend dispatch is automatic, so there is nothing left to convert between backends | `tests/golden/test_backend_parity_golden.py::test_converted_model_reproduces_the_committed_cpu_reference` |
| `ep.mace_cueq_to_e3nn` | `mace.cli.convert_cueq_e3nn:main` | `packages/mace-launcher/pyproject.toml` | DROP — idem, the reverse direction | `tests/backends/backend_parity.py::test_bidirectional_conversion` |
| `ep.mace_active_learning_md` | `mace.cli.active_learning_md:main` | `packages/mace-launcher/pyproject.toml` | DROP — out of v1.0 scope: an ASE MD loop a user writes in ~30 lines over the calculator, at the cost of MACE owning thermostat, timestep and trajectory I/O. The committee variance it consumes stays in `calculate` (§16); the release notes document the recipe | — |
...
| `train.downloads_dir` | `--downloads_dir` | `mace/tools/arg_parser.py:60` | MERGE — XDG cache-dir convention | `tests/unit/test_readout_and_directory_flags.py::test_the_downloads_directory_defaults_under_the_work_dir` + `tests/unit/test_readout_and_directory_flags.py::test_an_explicit_downloads_directory_is_kept` |
...
| `train.log_level` | `--log_level` | `mace/tools/arg_parser.py:90` | KEEP | `tests/unit/test_logging_setup.py` |
...
| `train.r_max` | `--r_max` | `mace/tools/arg_parser.py:151` | KEEP | `tests/unit/test_radial.py::test_polynomial_cutoff_is_exactly_zero_at_and_beyond_r_max` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
| `train.radial_type` | `--radial_type` | `mace/tools/arg_parser.py:154` | KEEP — bessel / gaussian / chebyshev | `tests/unit/test_radial.py::test_bessel_basis_values` + `tests/unit/test_radial.py::test_gaussian_basis_values` + `tests/unit/test_radial.py::test_chebychev_basis_values` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
| `train.num_radial_basis` | `--num_radial_basis` | `mace/tools/arg_parser.py:161` | KEEP | `tests/unit/test_radial.py::test_bessel_shape_and_dtype_contract` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
| `train.num_cutoff_basis` | `--num_cutoff_basis` | `mace/tools/arg_parser.py:167` | KEEP | `tests/unit/test_radial.py::test_polynomial_cutoff_is_one_at_zero_for_every_p` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
| `train.pair_repulsion` | `--pair_repulsion` | `mace/tools/arg_parser.py:173` | KEEP — the ZBL short-range term | `tests/unit/test_radial.py::test_zbl_matches_the_published_formula` + `tests/golden/test_tiny_anchors.py::test_the_repulsion_term_is_scaled_in_one_class_and_raw_in_the_other` + `tests/parity/test_radial_parity.py::test_zbl_parity` (v1 parity) |
| `train.distance_transform` | `--distance_transform` | `mace/tools/arg_parser.py:179` | KEEP | `tests/unit/test_radial.py::test_the_basis_sees_the_transformed_lengths` + `tests/unit/test_radial.py::test_the_cutoff_is_computed_before_the_distance_transform` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
| `train.apply_cutoff` | `--apply_cutoff` | `mace/tools/arg_parser.py:185` | KEEP | `tests/unit/test_radial.py::test_apply_cutoff_true_returns_the_product_and_no_envelope` + `tests/unit/test_radial.py::test_apply_cutoff_false_defers_the_envelope_to_the_consumer` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
| `train.use_last_readout_only` | `--use_last_readout_only` | `mace/tools/arg_parser.py:191` | MERGE — readout policy: once you declare which layers read out, 'only the last' is configuration, not a boolean | `tests/unit/test_readout_and_directory_flags.py::test_the_flag_leaves_only_the_last_readout` + `tests/unit/test_readout_and_directory_flags.py::test_the_two_models_disagree_on_the_energy` |
| `train.use_embedding_readout` | `--use_embedding_readout` | `mace/tools/arg_parser.py:197` | MERGE — idem ('also read the embedding layer') | `tests/unit/test_readout_and_directory_flags.py::test_the_flag_adds_a_readout_over_the_joint_embedding` + `tests/unit/test_readout_and_directory_flags.py::test_it_needs_the_joint_embedding_to_read_from` |
...
| `train.multi_processed_test` | `--multi_processed_test` | `mace/tools/arg_parser.py:484` | MERGE — the data layer infers sharding from the dataset; whether a test set is split across files is not something the user should have to declare and get wrong (today it is a bare `if` in `run_train.py`) | `tests/workflows/test_multi_processed_test_layout.py` (both layouts, from a real `mace_prepare_data` run, and both mismatches) |
...
| `train.pin_memory` | `--pin_memory` | `mace/tools/arg_parser.py:497` | KEEP | `tests/workflows/test_train_work_dir_and_workers.py::test_every_dataloader_is_told_whether_to_pin` + `::test_the_flag_is_the_value_the_loaders_are_given` |
...
| `train.skip_evaluate_heads` | `--skip_evaluate_heads` | `mace/tools/arg_parser.py:773` | KEEP | `tests/unit/test_error_table_heads.py` (which rows the flag removes, and the substring match it uses) + `tests/workflows/test_per_head_reporting.py::test_the_replay_head_is_left_out_of_the_final_table` (the default, on a real finetuning run) |
...
| `train.pseudolabel_replay_compute_stress` | `--pseudolabel_replay_compute_stress` | `mace/tools/arg_parser.py:553` | KEEP | `tests/unit/test_multihead_tools.py::test_forcing_it_adds_the_stress_the_model_predicts` |
...
| `prep.polarizability_key` | `--polarizability_key` | `mace/tools/arg_parser.py:1310` | MERGE — `mace data prepare` reads the same config section as `mace train` instead of redeclaring the flag | `tests/workflows/test_preprocess_keys.py::test_a_property_key_is_read_under_the_name_it_is_given` |
...
| `cli.fine_tuning_select.head_pt` | `--head_pt` | `mace/cli/fine_tuning_select.py:140` | MERGE — absorbed into the integrated fine-tuning pipeline (`mace train` with a fine-tuning config); selection stops being a separate CLI over a separate model load | `tests/workflows/test_finetuning_select_cli.py::test_the_heads_label_each_half` |
| `cli.fine_tuning_select.head_ft` | `--head_ft` | `mace/cli/fine_tuning_select.py:146` | MERGE — absorbed into the integrated fine-tuning pipeline (`mace train` with a fine-tuning config); selection stops being a separate CLI over a separate model load | `tests/workflows/test_finetuning_select_cli.py::test_the_heads_label_each_half` + `tests/workflows/test_finetuning_select_cli.py::test_no_head_key_is_written_when_none_is_given` |
...
| `cli.fine_tuning_select.weight_ft` | `--weight_ft` | `mace/cli/fine_tuning_select.py:159` | MERGE — absorbed into the integrated fine-tuning pipeline (`mace train` with a fine-tuning config); selection stops being a separate CLI over a separate model load | `tests/workflows/test_finetuning_select_cli.py::test_the_finetuning_weight_applies_to_the_finetuning_configs` |
| `cli.fine_tuning_select.weight_pt` | `--weight_pt` | `mace/cli/fine_tuning_select.py:165` | MERGE — absorbed into the integrated fine-tuning pipeline (`mace train` with a fine-tuning config); selection stops being a separate CLI over a separate model load | `tests/workflows/test_finetuning_select_cli.py::test_the_pretraining_weight_is_written_onto_the_selection` |
| `cli.fine_tuning_select.filter_atomic_numbers_pt` | `--filter_atomic_numbers_pt` | `mace/cli/fine_tuning_select.py:171` | MERGE — absorbed into the integrated fine-tuning pipeline (`mace train` with a fine-tuning config); selection stops being a separate CLI over a separate model load | `tests/workflows/test_finetuning_select_cli.py::test_the_atomic_number_filter_restricts_the_pool` |
...
| `model.ChebyshevBasisGeneral` | `ChebyshevBasisGeneral` | `mace/modules/extensions.py:1374` | KEEP — the magnetic-moment radial basis, living in `extensions.py`; **not** a `--radial_type` variant (`--radial_type` is `bessel` / `gaussian` / `chebyshev`, `arg_parser.py:155-161`). ARCH-1 collapses it and `ChebychevBasis` onto one `mace_torch.nn.radial.ChebyshevBasis`; its `include_constant` switch is part of the contract and its consumer is the magnetic family | `tests/unit/test_radial.py::test_chebychev_basis_values` + `tests/unit/test_radial.py::test_chebychev_ignores_r_max_and_diverges_outside_the_unit_interval` + `tests/parity/test_radial_parity.py::test_chebyshev_basis_parity_at_the_magnetic_configuration` (v1 parity) |
...
| `reg.RealAgnosticDensityInteractionBlock` | `RealAgnosticDensityInteractionBlock` — interaction_classes | `mace/modules/__init__.py:74` | KEEP — foundation-model architecture, not a research variant: MACE-MP-0b2 S/M/L and MACE-mh-0 use it in the first layer | `tests/unit/test_first_interaction_block.py::test_the_density_block_is_reachable_as_a_first_layer` |
...
| id | feature | source | disposition | pinned by |
| --- | --- | --- | --- | --- |
| `radial.BesselBasis` | `BesselBasis` | `mace/modules/radial.py:18` | KEEP — the default radial basis | `tests/unit/test_modules.py::test_bessel_basis` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
| `radial.ChebychevBasis` | `ChebychevBasis` | `mace/modules/radial.py:61` | KEEP — `--radial_type chebyshev`; in v1 the same class as `ChebyshevBasisGeneral` (`ChebyshevBasis`, misspelling dropped) | `tests/unit/test_radial.py::test_chebychev_basis_values` + `tests/parity/test_radial_parity.py::test_chebyshev_basis_parity_with_the_radial_type_class` (v1 parity) |
| `radial.GaussianBasis` | `GaussianBasis` | `mace/modules/radial.py:89` | KEEP — `--radial_type gaussian` | `tests/unit/test_radial.py::test_gaussian_basis_values` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
| `radial.PolynomialCutoff` | `PolynomialCutoff` | `mace/modules/radial.py:113` | KEEP — the envelope every basis is multiplied by | `tests/unit/test_modules.py::test_polynomial_cutoff` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
| `radial.ZBLBasis` | `ZBLBasis` | `mace/modules/radial.py:149` | KEEP — `--pair_repulsion`, a physics term rather than a basis | `tests/unit/test_radial.py::test_zbl_buffers_and_trainability` + `tests/parity/test_radial_parity.py::test_zbl_parity` (v1 parity) |
| `radial.AgnesiTransform` | `AgnesiTransform` | `mace/modules/radial.py:225` | KEEP — `--distance_transform agnesi` | `tests/unit/test_radial.py::test_agnesi_transform_values` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
| `radial.SoftTransform` | `SoftTransform` | `mace/modules/radial.py:285` | KEEP — `--distance_transform soft` | `tests/unit/test_radial.py::test_soft_transform_values` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
...
| `block.LinearNodeEmbeddingBlock` | `LinearNodeEmbeddingBlock` | `mace/modules/blocks.py:45` | KEEP — one-hot elements to node features, in every model | `tests/unit/test_models.py::test_mace` + `tests/parity/test_radial_parity.py::test_linear_node_embedding_parity_with_the_legacy_weight_rescaled` (v1 parity) |
| `block.RadialEmbeddingBlock` | `RadialEmbeddingBlock` | `mace/modules/blocks.py:395` | KEEP — basis x cutoff, in every model | `tests/unit/test_radial.py::test_the_basis_sees_the_transformed_lengths` + `tests/parity/test_radial_parity.py::test_radial_embedding_block_parity` (v1 parity) |
...
| `block.NonLinearBiasReadoutBlock` | `NonLinearBiasReadoutBlock` | `mace/modules/blocks.py:123` | KEEP — the biased readout the field models use | `tests/extensions/polar/test_polar_models.py::test_polar_mace_registers_all_trainable_parameters` (built by `PolarMACE.__init__` as `fukui_source_map`, so its weights are among the ones that must be claimed) |
| `block.GeneralNonLinearBiasReadoutBlock` | `GeneralNonLinearBiasReadoutBlock` | `mace/modules/blocks.py:314` | KEEP — idem, for `mace/modules/field_blocks.py` | `tests/extensions/polar/test_polar_models.py::test_energy_invariance_under_rotation_and_translation` (built by `AgnosticEmbeddedOneBodyVariableUpdate`, which the minimal polar model selects) |
...
| `block.RealAgnosticAttResidualInteractionBlock` | `RealAgnosticAttResidualInteractionBlock` | `mace/modules/blocks.py:1286` | MERGE — idem | `tests/unit/test_interaction_variants.py::test_the_attention_residual_interaction_builds_and_runs` |
...
| `block.MagneticRealAgnosticResidueSpinOrbitCoupledDensityInteractionBlock` | `MagneticRealAgnosticResidueSpinOrbitCoupledDensityInteractionBlock` | `mace/modules/blocks.py:1785` | MERGE — the residual SOC variant | `tests/extensions/magnetic/test_magnetic_interaction_variants.py::test_the_residual_soc_interaction_builds_and_runs` |
| `block.EquivariantProductBasisWithSelfMagmomBlock` | `EquivariantProductBasisWithSelfMagmomBlock` | `mace/modules/blocks.py:517` | KEEP — the product basis that carries the site's own moment | `tests/extensions/magnetic/test_magmace.py::test_magnetic_mace_registers_all_trainable_parameters` (built by `MagneticMACE.__init__` for every layer) |
...
| `out.calc.stress_comm` | `stress_comm` — committee | `mace/calculators/mace.py:718` | KEEP — committee output (per-model stresses). The committee is the part of `mace_active_learning_md` that MACE must keep guaranteeing (§1) | `tests/unit/test_calculator_units_and_spread.py::test_a_committee_reports_the_stress_of_the_members_it_averaged` |
| `out.calc.stress_var` | `stress_var` — committee | `mace/calculators/mace.py:718` | KEEP — committee output (stress variance). The committee is the part of `mace_active_learning_md` that MACE must keep guaranteeing (§1) | `tests/unit/test_calculator_units_and_spread.py::test_the_stress_variance_scales_as_the_square_of_the_conversion` + `tests/unit/test_calculator_units_and_spread.py::test_the_stress_spread_is_in_the_same_layout_as_the_stress` + `tests/unit/test_calculator_units_and_spread.py::test_the_members_are_symmetric_so_the_conversion_loses_nothing` |
| `out.calc.dipole_comm` | `dipole_comm` — committee | `mace/calculators/mace.py:718` | KEEP — committee output (per-model dipoles). The committee is the part of `mace_active_learning_md` that MACE must keep guaranteeing (§1) | `tests/unit/test_calculator_units_and_spread.py::test_a_dipole_committee_reports_its_spread` |
| `out.calc.dipole_var` | `dipole_var` — committee | `mace/calculators/mace.py:718` | KEEP — committee output (dipole variance). The committee is the part of `mace_active_learning_md` that MACE must keep guaranteeing (§1) | `tests/unit/test_calculator_units_and_spread.py::test_a_dipole_committee_reports_its_spread` + `tests/unit/test_calculator_units_and_spread.py::test_the_dipole_spread_keeps_the_layout_of_the_dipole` |
...
| `kwarg.dispersion_xc` | `mace_mp(dispersion_xc=…)` — the functional the D3 parameters are taken from | `mace/calculators/foundations_models.py` | KEEP | `tests/unit/test_foundations_models.py::test_dispersion_xc_is_forwarded` + `tests/unit/test_foundations_models.py::test_the_default_functional_is_pbe` |
| `kwarg.dispersion_cutoff` | `mace_mp(dispersion_cutoff=…)` | `mace/calculators/foundations_models.py` | KEEP | `tests/unit/test_foundations_models.py::test_dispersion_cutoff_is_forwarded` + `tests/unit/test_foundations_models.py::test_the_two_cutoffs_are_separate_knobs` |
