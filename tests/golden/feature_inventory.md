...
**A cell may name a pin in each stack.** A row whose behaviour the rewrite has reimplemented carries
its legacy pin first and the v1 one after it, under `packages/<distribution>/tests/`. The second pin
says the capability exists on the other side too, which is the question the migration has to answer
per row rather than per phase. Both are resolved by the checker, node id included, and a whole
package suite is as coarse a pin as a whole legacy tier.

The order is not cosmetic and the checker enforces it: a v1 pin may not stand alone. This file is
about what the frozen tree does, and a test in the rewrite cannot say whether the rewrite still
matches it. Dropping the legacy pin would turn the row from a claim the oracle checks into a claim
the rewrite makes about itself.
...
| `train.energy_key` | `--energy_key` | `mace/tools/arg_parser.py:672` | MERGE — property-key convention of the observable spec | `tests/unit/test_data_utils.py::test_custom_property_keys_round_trip` + `tests/unit/test_data_utils.py::test_keydict_derives_one_cli_argument_per_member` + `packages/mace-core/tests/test_data_configuration.py::test_a_custom_file_key_reads_only_when_the_specification_names_it` |
...
| `train.virials_key` | `--virials_key` | `mace/tools/arg_parser.py:684` | MERGE — idem | `tests/unit/test_data_utils.py::test_custom_property_keys_round_trip` + `tests/unit/test_data_utils.py::test_keydict_derives_one_cli_argument_per_member` + `packages/mace-core/tests/test_data_configuration.py::test_all_thirteen_default_keys_survive_a_write_and_a_read` |
...
| `train.charges_key` | `--charges_key` | `mace/tools/arg_parser.py:726` | MERGE — idem | `tests/unit/test_data_utils.py::test_custom_property_keys_round_trip` + `tests/unit/test_data_utils.py::test_keydict_derives_one_cli_argument_per_member` + `packages/mace-core/tests/test_data_configuration.py::test_all_thirteen_default_keys_survive_a_write_and_a_read` |
| `train.head_key` | `--head_key` | `mace/tools/arg_parser.py:720` | MERGE — idem | `tests/unit/test_data_utils.py::test_custom_property_keys_round_trip` + `tests/unit/test_data_utils.py::test_keydict_derives_one_cli_argument_per_member` + `packages/mace-core/tests/test_data_configuration.py::test_the_head_name_is_stamped_on_every_structure` |
| `train.elec_temp_key` | `--elec_temp_key` | `mace/tools/arg_parser.py:732` | KEEP — graph-level input feature | `tests/workflows/test_embedding_train.py::test_run_train_with_elec_temp` + `tests/unit/test_lmdb_database.py::test_lmdb_dataset_honors_key_specification` + `packages/mace-core/tests/test_data_configuration.py::test_the_graph_level_inputs_are_per_structure_keys` + `packages/mace-core/tests/test_data_configuration.py::test_a_custom_file_key_reads_only_when_the_specification_names_it` |
| `train.total_spin_key` | `--total_spin_key` | `mace/tools/arg_parser.py:738` | KEEP — idem | `tests/unit/test_lmdb_database.py::test_lmdb_dataset_honors_key_specification` + `packages/mace-core/tests/test_data_configuration.py::test_the_graph_level_inputs_are_per_structure_keys` + `packages/mace-core/tests/test_data_configuration.py::test_a_custom_file_key_reads_only_when_the_specification_names_it` |
| `train.total_charge_key` | `--total_charge_key` | `mace/tools/arg_parser.py:744` | KEEP — idem | `tests/unit/test_lmdb_database.py::test_lmdb_dataset_honors_key_specification` + `packages/mace-core/tests/test_data_configuration.py::test_the_graph_level_inputs_are_per_structure_keys` + `packages/mace-core/tests/test_data_configuration.py::test_a_custom_file_key_reads_only_when_the_specification_names_it` |
| `train.embedding_specs` | `--embedding_specs` | `mace/tools/arg_parser.py:750` | KEEP — categorical / graph-level embeddings | `tests/workflows/test_embedding_train.py::test_run_train_with_atom_embed` + `tests/unit/test_data_utils.py::test_update_keyspec_from_kwargs_embedding_specs` + `packages/mace-core/tests/test_data_configuration.py::test_an_embedding_feature_lands_in_the_half_its_per_names` + `packages/mace-core/tests/test_data_configuration.py::test_an_embedding_feature_reads_through_a_real_file` |
...
| id | feature | source | disposition | pinned by |
|---|---|---|---|---|
| `train.magmom_key` | `--magmom_key` | `mace/tools/arg_parser.py:708` | MERGE — property-key convention; the default `REF_magmom` extends the on-disk data contract of §15 | `tests/extensions/magnetic` + `packages/mace-core/tests/test_data_configuration.py::test_the_magnetic_arrays_resolve_with_no_magnetic_flag_set` + `packages/mace-core/tests/test_data_configuration.py::test_a_custom_file_key_reads_only_when_the_specification_names_it` |
| `train.magforces_key` | `--magforces_key` | `mace/tools/arg_parser.py:714` | MERGE — idem, default `REF_magforces` | `tests/extensions/magnetic` + `packages/mace-core/tests/test_data_configuration.py::test_the_magnetic_arrays_resolve_with_no_magnetic_flag_set` + `packages/mace-core/tests/test_data_configuration.py::test_a_custom_file_key_reads_only_when_the_specification_names_it` |
...
| id | feature | source | disposition | pinned by |
|---|---|---|---|---|
| `key.ENERGY` | `ENERGY` = `"REF_energy"` | `mace/tools/default_keys.py:7` | KEEP — the default name is part of the data contract; a rename needs an explicit old→new mapping in the release notes | `tests/unit/test_data_utils.py::test_default_keys_are_exactly_these_thirteen` + `tests/unit/test_data_utils.py::test_custom_property_keys_round_trip` + `packages/mace-core/tests/test_data_configuration.py::test_the_default_keys_are_exactly_these_thirteen` + `packages/mace-core/tests/test_data_configuration.py::test_all_thirteen_default_keys_survive_a_write_and_a_read` |
...
| `key.DIPOLE` | `DIPOLE` = `"dipole"` | `mace/tools/default_keys.py:11` | KEEP — the default name is part of the data contract; a rename needs an explicit old→new mapping in the release notes | `tests/unit/test_data_utils.py::test_default_keys_are_exactly_these_thirteen` + `tests/unit/test_data_utils.py::test_custom_property_keys_round_trip` + `packages/mace-core/tests/test_data_configuration.py::test_the_default_keys_are_exactly_these_thirteen` + `packages/mace-core/tests/test_data_configuration.py::test_the_default_dipole_key_cannot_be_read_back_from_a_file` |
