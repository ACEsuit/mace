...
cd $ROOT_DIR
mace_run_train \
    --name="stress${stress}_nc${NUM_CHANNEL}_nr${NUM_RADIAL}_MLP${MLP_IRREPS}_b${REAL_BATCH_SIZE}_lr$2_${conf_str}_intfirst-${int_first}_int-${int}x${num_int}i_maxL${max_L}_maxell${max_ell}_loss${loss}" \
    --loss=${loss} \
    --energy_weight=1 \
    --forces_weight=10 \
    --compute_stress=True \
