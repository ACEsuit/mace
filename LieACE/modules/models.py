...
class DipoleBOTNet(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
        ...
        self.interactions.append(inter)
        self.readouts.append(LinearDipoleReadoutBlock(inter.irreps_out))
        ...
        for i in range(num_interactions - 1):
            ...
            self.interactions.append(inter)
            if i == num_interactions - 2:
                self.readouts.append(NonLinearDipoleReadoutBlock(inter.irreps_out, MLP_irreps, gate))
            else:
                self.readouts.append(LinearDipoleReadoutBlock(inter.irreps_out))
    ...
    def forward(self, data: AtomicData, training=False) -> Dict[str, Any]:
        ...
        for interaction, readout in zip(self.interactions, self.readouts):
            node_feats = interaction(node_attrs=data.node_attrs,
                                     ...
                                     edge_feats=edge_feats,
                                     edge_index=data.edge_index)
            node_energies = readout(node_feats)[:,0].squeeze(-1)  # [n_nodes, ]
            node_dipoles = readout(node_feats)[:,1:].squeeze(-1) 
            energy = scatter_sum(src=node_energies, index=data.batch, dim=-1, dim_size=data.num_graphs)  # [n_graphs,]
            energies.append(energy)
        ...
        total_dipoles= torch.sum(contributions_dipoles, dim=-1) 
...
class ScaleShiftDipoleBOTNet(DipoleBOTNet):
    def __init__(
        self,
        atomic_inter_scale: float,
        atomic_inter_shift: float,
        ...
        **kwargs,
        ...
        super().__init__(**kwargs)
        self.scale_shift_energy = ScaleShiftBlock(scale=atomic_inter_scale, shift=atomic_inter_shift)
        self.scale_shift_dipoles = ScaleShiftBlock(scale=dipole_scale, shift=dipole_shift)
    ...
    def forward(self, data: AtomicData, training=False) -> Dict[str, Any]:
        ...
        # Sum over interactions
        node_inter_es = torch.sum(torch.stack(node_es_list, dim=0), dim=0)  # [n_nodes, ]
        node_inter_es = self.scale_shift_energy(node_inter_es)
