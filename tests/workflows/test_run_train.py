...
def test_run_train_foundation_multihead(tmp_path, fitting_configs):
    ...
    try:
        completed_process = run_mace_train(mace_params, capture_output=True, text=True)
...
def test_run_train_multihead_replay_custom_finetuning(
    ...
    p = run_mace_train(foundation_params)
    assert p.returncode == 0
    ...
    p = run_mace_train(finetuning_params)
    assert p.returncode == 0
...
def fixture_mini_foundation_model(tmp_path_factory, pretraining_configs_3_elems):
    ...
    try:
        ...
        os.chdir(tmp_path)
        p = run_mace_train(
            foundation_params, env_extra={"XDG_CACHE_HOME": cache_home}
        )
        os.chdir(orig_dir)
    except Exception:
        os.chdir(orig_dir)
    ...
    return tmp_path / "foundation.model", cache_home
...
def test_run_train_multihead_replay_filtered_pt_data(
    ...
    with monkeypatch.context() as m:
        m.chdir(tmp_path)
        # create fake cached MP data via XDG_CACHE_HOME
        p = run_mace_train(
            finetuning_params,
            env_extra={"XDG_CACHE_HOME": mini_foundation_model[1]},
        )
    assert p.returncode == 0
...
def test_run_train_real_pt_data_ratio(
    ...
    with monkeypatch.context() as m:
        m.chdir(tmp_path)
        p = run_mace_train(finetuning_params, capture_output=True, env_extra=env_extra)
