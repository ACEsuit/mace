...
# NOTE: this file keeps its own sample_configs fixture (only 10 rattled
# configs, no print), so it is not the canonical fitting_configs fixture from
# tests/conftest.py
...
def test_preprocess_data(tmp_path, sample_configs):
    ...
    p = run_mace_train(preprocess_params, script=preprocess_data)
...
def test_preprocess_config(tmp_path, sample_configs):
    ...
    p = run_mace_train({}, extra_argv=["--config", str(filename)], script=preprocess_data)
    assert p.returncode == 0
