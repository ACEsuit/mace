...
| `ep.mace_plot_train` | `mace.cli.plot_train:main` | `setup.cfg` | KEEP — reduced: the basic plot subcommand stays (per-head loss curves, cheap over a structured log); `--plot_interaction_e` goes (§3.1) | ⚠️ gap (smoke: parse a real results log, write a file) |
...
| `train.work_dir` | `--work_dir` | `mace/tools/arg_parser.py:39` | KEEP | ⚠️ gap (no test passes `--work_dir`; every run sets the four directories below instead) |
...
| `train.return_electrostatic_potentials` | `--return_electrostatic_potentials` | `mace/tools/arg_parser.py:377` | MERGE — an observable declared in the output spec, not a model flag | ⚠️ gap (add a case to `tests/golden/test_polar_foundation.py`) |
...
| `train.num_workers` | `--num_workers` | `mace/tools/arg_parser.py:491` | KEEP | ⚠️ gap (perf knob; conscious downgrade candidate) |
...
| `train.swa_energy_weight` | `--swa_energy_weight` `--stage_two_energy_weight` | `mace/tools/arg_parser.py:827` | MERGE — per-stage schedules; arbitrary stages replace the two-stage special case, and the `swa` spelling dies with the namespace | ⚠️ gap (no test sets a stage-two loss weight; `--swa_energy_weight` only takes effect after the swap) |
| `train.swa_forces_weight` | `--swa_forces_weight` `--stage_two_forces_weight` | `mace/tools/arg_parser.py:802` | MERGE — idem | ⚠️ gap (no test sets a stage-two loss weight; `--swa_forces_weight` only takes effect after the swap) |
| `train.swa_virials_weight` | `--swa_virials_weight` `--stage_two_virials_weight` | `mace/tools/arg_parser.py:838` | MERGE — idem | ⚠️ gap (no test sets a stage-two loss weight; `--swa_virials_weight` only takes effect after the swap) |
| `train.swa_stress_weight` | `--swa_stress_weight` `--stage_two_stress_weight` | `mace/tools/arg_parser.py:849` | MERGE — idem | ⚠️ gap (no test sets a stage-two loss weight; `--swa_stress_weight` only takes effect after the swap) |
...
| `prep.virials_key` | `--virials_key` | `mace/tools/arg_parser.py:1292` | MERGE — `mace data prepare` reads the same config section as `mace train` instead of redeclaring the flag | ⚠️ gap (preprocess path) |
...
| `prep.dipole_key` | `--dipole_key` | `mace/tools/arg_parser.py:1304` | MERGE — `mace data prepare` reads the same config section as `mace train` instead of redeclaring the flag | ⚠️ gap (preprocess path) |
...
| `prep.charges_key` | `--charges_key` | `mace/tools/arg_parser.py:1316` | MERGE — `mace data prepare` reads the same config section as `mace train` instead of redeclaring the flag | ⚠️ gap (preprocess path) |
...
| `cli.eval_configs.default_dtype` | `--default_dtype` | `mace/cli/eval_configs.py:45` | MERGE — `PrecisionConfig` | `tests/workflows/test_cli_contracts.py::test_eval_at_float32_reproduces_float64_within_the_fp32_row` + `tests/workflows/test_cli_contracts.py::test_eval_refuses_a_dtype_that_disagrees_with_the_checkpoint` |
...
| `cli.eval_configs.return_node_energies` | `--return_node_energies` | `mace/cli/eval_configs.py:95` | KEEP — idem | `tests/workflows/test_cli_contracts.py::test_eval_node_energies_sum_to_the_total_energy` + `tests/workflows/test_cli_contracts.py::test_eval_node_energies_cannot_handle_structures_of_different_sizes` |
...
| `cli.fine_tuning_select.device` | `--device` | `mace/cli/fine_tuning_select.py:126` | MERGE — absorbed into the integrated fine-tuning pipeline (`mace train` with a fine-tuning config); selection stops being a separate CLI over a separate model load | ⚠️ gap (no selection test passes `--device`) |
...
| `cli.fine_tuning_select.seed` | `--seed` | `mace/cli/fine_tuning_select.py:182` | MERGE — absorbed into the integrated fine-tuning pipeline (`mace train` with a fine-tuning config); selection stops being a separate CLI over a separate model load | ⚠️ gap (no selection test passes `--seed`, so the sampling is not pinned as reproducible) |
| `cli.fine_tuning_select.default_dtype` | `--default_dtype` | `mace/cli/fine_tuning_select.py:133` | MERGE — `PrecisionConfig` | ⚠️ gap (port `tests/workflows/test_finetuning_select.py`) |
| `cli.fine_tuning_select.config` | `--config` | `mace/cli/fine_tuning_select.py:83` | MERGE — the v1 config system | ⚠️ gap (no test exercises the YAML path) |

### `mace_plot_train` — `mace/cli/plot_train.py` (8)

| id | feature | source | disposition | pinned by |
|---|---|---|---|---|
| `cli.plot_train.path` | `--path` | `mace/cli/plot_train.py:78` | KEEP | ⚠️ gap (plot smoke: parse a real results log, write a file) |
| `cli.plot_train.min_epoch` | `--min_epoch` | `mace/cli/plot_train.py:81` | KEEP | ⚠️ gap (idem) |
| `cli.plot_train.linear` | `--linear` | `mace/cli/plot_train.py:93` | KEEP | ⚠️ gap (idem) |
| `cli.plot_train.error_bars` | `--error_bars` | `mace/cli/plot_train.py:100` | KEEP | ⚠️ gap (idem) |
...
| `reg.abs` | `abs` — gate_dict | `mace/modules/__init__.py:97` | KEEP | ⚠️ gap (add a case to `tests/unit/test_models.py`) |
| `reg.None` | `None` — gate_dict | `mace/modules/__init__.py:100` | KEEP — the string `"None"`, meaning no gate | ⚠️ gap (add a case to `tests/unit/test_models.py`) |
...
| `extra.fpsample` | `[fpsample]` | `setup.cfg` | KEEP — fast farthest-point sampling for fine-tuning selection | ⚠️ gap (port `tests/workflows/test_finetuning_select.py`) |
...
| `out.model.external_field` | `external_field` — first declared by `PolarMACE` | `mace/modules/extensions.py:1337` | KEEP — idem | ⚠️ gap (idem) |
...
| `pkg.visualise_train` | `mace/cli/visualise_train.py` — the plotting support module behind `mace_plot_train`, with no entry point of its own | `mace/cli/visualise_train.py` | KEEP — follows its CLI (§1) | ⚠️ gap (with the plot smoke) |
| `pkg.public_import_surface` | The public Python API downstream projects import — `mace.modules.MACE`, `mace.data.AtomicData`, `mace.cli.run_train.run(args)`, … | `mace/` | DROP — a deliberate break: v1 defines a new public API, and the release notes document the old→new equivalences rather than aliasing them | — (deliberate break) |
...
| `pkg.results_log_format` | The per-epoch results log (`results/*.txt`), which `mace_plot_train` parses and users script against | `mace/tools/train.py` | KEEP — becomes a structured (typed) log; the current line format is not a promise v1 makes | ⚠️ gap (resolved together with the plot smoke) |
