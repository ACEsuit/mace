...
| `train.multi_processed_test` | `--multi_processed_test` | `mace/tools/arg_parser.py:484` | MERGE — the data layer infers sharding from the dataset; whether a test set is split across files is not something the user should have to declare and get wrong (today it is a bare `if` in `run_train.py`) | ⚠️ gap (the two branches differ only in the on-disk layout of the test set: flat `*_test.h5` files against one folder of shards per set, both read inline in `run_train.run`. Testing it needs a preprocessed dataset written twice, in both layouts, inside a full run, which is why it is still here) |
