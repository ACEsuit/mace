...
This is a copy of the MACE repository, with added functionality for the lammps-MLIAP plugin.

#### Python installation
To install this, on top of the standard MACE installation (torch, e3nn, etc.) you will require:

- cuequivariance (base, torch, torch backend -- see [here](https://github.com/NVIDIA/cuEquivariance) )
- cupy-cuda12x
- the lammps generated python package (see below)

It might be best to use `torch<=2.5.0`, because of the new model loading warnings.
Also, I think some dependency of mace wants numpy<2, but >2 for cueq seems to be fine.

#### LAMMPS compilation
##### General ideas
This plugin requires the [`develop`](https://github.com/lammps/lammps) branch of lammps.

To install lammps to be compatible with this you should follow [this](https://docs.lammps.org/pair_mliap.html) 
with the options for kokkos and python.
In practice this means all the KOKKOS flags, plus `PKG_ML-IAP=ON` with `MLIAP_ENABLE_PYTHON=ON` and `PKG_ML-SNAP=ON`, 
` PKG_PYTHON=ON` and `BUILD_SHARED_LIBS=ON` to create the python package.
After compiling, we need to run `make install-python` to create the wheel and install it.

##### A specific example
(which might not work for you)

- Download lammps with `git clone https://github.com/lammps/lammps.git`
- Create your build directory
- Copy `cmake/presets/kokkos-cuda.cmake` to your build dir and modify the architecture as needed (the `set(Kokkos_ARCH_XXXX ON CACHE BOOL "" FORCE)` with the proper arch from [here](https://docs.lammps.org/Build_extras.html#kokkos))
- If you are planning to use python from a virtualenv, it is a good idea to have it enabled now, so it will select the right executable
- Run `cmake`. Your command should be something like:
```
cmake -C kokkos-cuda.cmake -D CMAKE_BUILD_TYPE=Release -D CMAKE_INSTALL_PREFIX=$(pwd) \
  -D BUILD_MPI=ON -D PKG_ML-IAP=ON -D PKG_ML-SNAP=ON \
  -D MLIAP_ENABLE_PYTHON=ON -D PKG_PYTHON=ON -D BUILD_SHARED_LIBS=ON \
  ../cmake
```
- Make with `make -j 8` (or however many cores you want to use)
- Create the wheel and install it in your python environment with `make install-python`


#### Exporting the model
To create a model, you can use the original `mace/cli/create_lammps_model.py` tool with the new keyword `--format=mliap`.
This should be compatible with most MACE models, including old checkpoints (let me know if it fails).
This should generate a `*-mliap_lammps.pt` model file compatible with the ML-IAP unified potential.

#### LAMMPS input
In your lammps input file, you can use this potential by specifying:
```
pair_style      mliap unified YOUR_MODEL-mliap_lammps.pt 0
pair_coeff      * * <your list of species>
```
where `YOUR_MODEL-mliap_lammps.pt` of course is the model you just saved, and `<your list of species>` 
needs to be a subset of the species you trained on, with the order you want to use inside lammps.

The kokkos command line options I used for testing are:
```lmp -k on g 1 -sf kk -pk kokkos newton on neigh half -in your_input.in```
You can change the `1` to the number of GPUs you want to use (assuming the rest is set up correctly).

#### Caveats
- The plugin is in a beta state, not thoroughly tested. Please let me know if you notice any issue (especially discrepancies with the original MACE) or require more features.
- The plugin ~~only works in fp32 precision! I'm looking into this, but there seem to be issues with the message passing.~~ now works in both fp32 and fp64 precision.
- The plugin only works with kokkos on GPU (it should be what we need anyway).
- The plugin should work on multiple GPUs with the usual kokkos flags. Please let me know if you notice any issues with this.
- The plugin uses cuequivariance by default (for symmetric contraction and channelwise), there is no way to disable this for now.
- The plugin does not support multiple model heads.
- It is possible to print the time/batch from within lammps, by setting the environment variable `MACE_TIME=true`
