...
def build_default_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=[
            "InvariantMultiACE",
            "scale_shift_non_linear",
            "BOTNet",
            "scale_shift_BOTNet",
            "FixedChargedDipoleBOTNet",
        ],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_interactions", help="number of interactions", type=int, default=3
    )
    parser.add_argument(
        "--num_interactions_dipole", help="number of interactions", type=int, default=3
    )
    parser.add_argument(
        "--MLP_irreps",
    ...
    parser.add_argument(
        "--hidden_irreps_dipole",
        help="irreps for hidden node states for dipole model",
        type=str,
        default="32x0e",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=[
            "std_scaling",
            "rms_forces_scaling",
            "qq_rms_forces_scaling",
            "qq_mu_mu_rms_forces_scaling",
        ],
    )
    parser.add_argument(
        "--num_avg_neighbors",
    ...
    parser.add_argument(
        ...
        help="dataset name",
        type=str,
        choices=["iso17", "rmd17", "3bpa", "acac", "ethanol", "water"],
        required=True,
    )
    ...
    parser.add_argument(
        ...
        help="type of loss",
        default="weighted",
        choices=["ef", "ace", "weighted", "dipole"],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dipole_weight", help="weight of energy loss", type=float, default=1.0
    )
    parser.add_argument(
        "--optimizer",
    ...
    parser.add_argument(
        "--weight_decay", help="weight decay (L2 penalty)", type=float, default=5e-5
    )
    parser.add_argument(
        "--amsgrad",
    ...
    parser.add_argument(
        "--scheduler_partience", help="Learning rate factor", type=int, default=50
    )
    parser.add_argument(
        "--lr_scheduler_gamma",
    ...
    parser.add_argument(
        "--max_num_epochs_dipole",
        help="Maximum number of epochs for dipole",
        type=int,
        default=2048,
    )
    parser.add_argument(
    ...
    try:
        ...
        if value != "None":
            raise argparse.ArgumentTypeError(
                f"{value} is an invalid value (int or None)"
            ) from None
        return None
