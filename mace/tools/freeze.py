...
def freeze_layers(model: torch.nn.Module, n: int) -> None:
    ...
    if abs(n) > num_layers:
        logging.warning(f"Requested {n} layers, but model only has {num_layers}. Adjusting `n` to fit the model.")
...
def freeze_param(model: torch.nn.Module, n: int) -> None:
    ...
    if abs(n) > num_params:
        logging.warning(f"Requested {n} parameter groups, but model only has {num_params}. Adjusting `n` to fit the model.")
        n = num_params if n > 0 else -num_params

    freeze_until = n if n > 0 else num_params + n

    for idx, (name, param) in enumerate(param_list):
        param.requires_grad = idx >= freeze_until  # Freeze first `n` or last `|n|` parameters

    logging.info(f"Froze {freeze_until if n > 0 else num_params - freeze_until} parameter groups.")