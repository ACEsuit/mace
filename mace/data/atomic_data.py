...
class AtomicData(torch_geometric.data.Data):
    ...
    def from_config(
        ...
        **kwargs,  # pylint: disable=unused-argument
        ...
        # Convert rigid-particle orientation and shape to lab-frame symmetric
        # rank-2 tensors. Quaternions are scalar-first: (w, x, y, z).
        ...
        zero_irreps = torch.zeros(
            (num_atoms, 6),
            dtype=torch.get_default_dtype(),
        )

        quaternion_tensor = (
            torch.as_tensor(quaternions, dtype=torch.get_default_dtype())
            if quaternions is not None
            else None
        )
        diameter_tensor = (
            torch.as_tensor(diameters, dtype=torch.get_default_dtype())
            if diameters is not None
            else None
        )

        if quaternion_tensor is not None and diameter_tensor is not None:
            inertia_tensor = ellipsoid_inertia_tensor(
                quaternion_tensor,
                diameter_tensor,
            )
            inertia_irreps = cartesian_tensor_to_irreps(inertia_tensor)

            steric_extent_tensor_value = steric_extent_tensor(
                quaternion_tensor,
                diameter_tensor,
            )
            steric_extent_irreps = cartesian_tensor_to_irreps(
                steric_extent_tensor_value
            )
        else:
            inertia_tensor = zero_tensor
            inertia_irreps = zero_irreps
            steric_extent_tensor_value = zero_tensor
            steric_extent_irreps = zero_irreps

        gyration_principal = config.properties.get("gyration_principal")
        if quaternion_tensor is not None and gyration_principal is not None:
            gyration_tensor = principal_tensor_from_values(
                quaternion_tensor,
                torch.as_tensor(
                    gyration_principal,
                    dtype=torch.get_default_dtype(),
            ...
            gyration_irreps = cartesian_tensor_to_irreps(gyration_tensor)
        elif quaternion_tensor is not None and diameter_tensor is not None:
            gyration_tensor = ellipsoid_gyration_tensor(
                quaternion_tensor,
                diameter_tensor,
            )
            gyration_irreps = cartesian_tensor_to_irreps(gyration_tensor)
        else:
            gyration_tensor = zero_tensor
            gyration_irreps = zero_irreps

        steric_extent_principal = config.properties.get("steric_extent_principal")
        if quaternion_tensor is not None and steric_extent_principal is not None:
            steric_extent_tensor_value = principal_tensor_from_values(
                quaternion_tensor,
                torch.as_tensor(
        ...
        quadrupole_principal = config.properties.get(
            "electrostatic_quadrupole_principal"
        )
        if quaternion_tensor is not None and quadrupole_principal is not None:
            electrostatic_quadrupole_tensor = principal_tensor_from_values(
                quaternion_tensor,
                torch.as_tensor(
                    quadrupole_principal,
                    dtype=torch.get_default_dtype(),
                ),
            )
            electrostatic_quadrupole_irreps = quadrupole_tensor_to_irreps(
                electrostatic_quadrupole_tensor
            )
        else:
            electrostatic_quadrupole_tensor = zero_tensor
            electrostatic_quadrupole_irreps = zero_irreps

        cls_kwargs["inertia_tensor"] = inertia_tensor
        cls_kwargs["inertia_irreps"] = inertia_irreps
        cls_kwargs["gyration_tensor"] = gyration_tensor
        cls_kwargs["gyration_irreps"] = gyration_irreps
        cls_kwargs["steric_extent_tensor"] = steric_extent_tensor_value
        cls_kwargs["steric_extent_irreps"] = steric_extent_irreps
        cls_kwargs["electrostatic_quadrupole_tensor"] = electrostatic_quadrupole_tensor
        cls_kwargs["electrostatic_quadrupole_irreps"] = electrostatic_quadrupole_irreps
