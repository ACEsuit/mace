...
class Eigenvalues_MACE(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
        ...
        self.readouts = torch.nn.ModuleList()
        self.readouts.append(LinearReadoutBlock(hidden_irreps))
        self.bond_interactions = torch.nn.ModuleList()
        bond_inter = MatrixFunctionBlock(
            node_feats_irreps=hidden_irreps,
    ...
    def forward(
        ...
        energies = [e0]
        node_energies_list = [node_e0]
        for interaction, product, readout, bond_interaction in zip(
            self.interactions, self.products, self.readouts, self.bond_interactions
        ):
            node_feats, sc = interaction(
                node_attrs=data["node_attrs"],
                node_feats=node_feats,
...
class ScaleShiftEigenvalues_MACE(Eigenvalues_MACE):
    def __init__(
        self,
        atomic_inter_scale: float,
        atomic_inter_shift: float,
