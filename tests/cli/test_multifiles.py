...
def test_multifile_training(default_dtype, tmp_path) -> None:
    ...
    iso_atoms_file = (tmp_path / "isolated_atoms.xyz").as_posix()
    config_file = (tmp_path / "config.yaml").as_posix()
    e0s_file = (tmp_path / "e0s.json").as_posix()

    h5_folder = (tmp_path / "h5_data")
    lmdb_folder1 = (tmp_path / "lmdb_data1_lmdb")
    lmdb_folder2 = (tmp_path / "lmdb_data2_lmdb")
    ...
    seeds = rng.integers(0, 10000, size=5)
    ...
    all_atoms_xyz = (tmp_path / "all_atoms_for_h5.xyz").as_posix()
    ...
    config = {
        ...
        "heads": {
            "head1": {
                "train_file": [lmdb_folder1.as_posix(), xyz_file1],
            ...
            "head2": {
                "train_file": [(h5_folder / "train").as_posix(), xyz_file2],
    ...
    cmd = [run_train_script, "--config", config_file]
    ...
    model_path = (model_dir / "multifile_test.model").as_posix()
...
def test_multiple_xyz_per_head(tmp_path) -> None:
    ...
    valid_xyz_files = [
        (tmp_path / f"valid_data{i}.xyz").as_posix() for i in range(1, 3)
    ]  # 2 valid files
    test_xyz_files = [
        (tmp_path / f"test_data{i}.xyz").as_posix() for i in range(1, 3)
    ]  # 2 test files
    ...
    # Create test data for each format
    rng = np.random.default_rng(42)
    seeds = rng.integers(0, 10000, size=10)  # More seeds for multiple files
    ...
    # Run training with subprocess
    cmd = [run_train_script, f"--config={config_file}"]
    ...
    # Check that model was created
    model_path = (model_dir / "multi_xyz_test.model").as_posix()
    assert os.path.exists(model_path), f"Model was not created at {model_path}"
...
def test_single_xyz_per_head(tmp_path) -> None:
    ...
    valid_xyz_files = [
        (tmp_path / f"valid_data{i}.xyz").as_posix() for i in range(1, 2)
    ]  # 2 valid files
    test_xyz_files = [
        (tmp_path / f"test_data{i}.xyz").as_posix() for i in range(1, 2)
    ]  # 2 test files
...
def test_multihead_finetuning_different_formats(tmp_path) -> None:
    ...
    h5_atoms_xyz = (tmp_path / "h5_atoms.xyz").as_posix()
    ...
    all_atoms_h5_xyz = (tmp_path / "all_atoms_for_h5.xyz").as_posix()
    ...
    heads = {
        ...
        "h5_head": {
            "train_file": (h5_folder / "train").as_posix(),
            "valid_file": (h5_folder / "val").as_posix(),
            "energy_key": "REF_energy",
            "forces_key": "REF_forces",
            "stress_key": "REF_stress",
            "E0s": e0s_file,
